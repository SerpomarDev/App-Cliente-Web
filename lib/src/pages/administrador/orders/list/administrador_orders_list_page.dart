...
import 'package:serpomar_client/src/pages/administrador/orders/list/administrador_orders_list_controller.dart';
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text('ORDEN #${order.id}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.white)),
                SizedBox(height: 10),
                ...
                Text('Contenedor: ${order.address?.address ?? ''}'),
                Text('Cita en puerto: ${order.citaPuerto ?? 'Sin asignar'}'),
                Text(order.status == 'FINALIZADOS' ? 'Finalizado el: ${formatDateTime(order.update_at)}' : 'Finalizado el: En progreso'),
                SizedBox(height: 10),
                Text('Mas información:', style: TextStyle(fontWeight: FontWeight.bold)),
                ...order.products!.map((Product product) {
                  return Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('# Contenedores: ${product.name ?? 'Sin nombre'}: ${product.quantity ?? 0} unidades'),
