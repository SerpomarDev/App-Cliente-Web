...
import 'package:serpomar_client/src/pages/asignacion/address/map/asignacion_address_map_page.dart';
...
class AsignacionAddressCreateController extends GetxController {
  ...
  AsignacionAddressListController asignacionAddressListController = Get.find();

  void openGoogleMaps(BuildContext context) async {
    Map<String, dynamic> refPointMap = await showMaterialModalBottomSheet(
        context: context,
        builder: (context) => AsignacionAddressMapPage(),
        isDismissible: false,
        enableDrag: false
    );
  ...
  void createAddress() async {
    ...
    if (isValidForm(addressName, neighborhood)) {
      ...
      if (responseApi.success == true) {
        ...
        asignacionAddressListController.update();
